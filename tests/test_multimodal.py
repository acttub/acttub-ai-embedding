...
def test_load_ignores_legacy_text_vectors(tmp_path):
    # 이전(3-모달) 인덱스에 text_vectors가 있어도 2-모달로 안전하게 로드된다.
    p = tmp_path / "legacy.npz"
    np.savez(
        str(p),
        ids=np.array(["a", "b"]),
        video_vectors=np.stack([_v(1, 0), _v(0, 1)]),
        audio_vectors=np.stack([_v(0, 1), _v(1, 0)]),
        text_vectors=np.stack([_v(1, 0), _v(0, 1)]),
    )
    loaded = MultiModalReferenceDB.load(str(p))
    res = loaded.search(_v(1, 0), _v(1, 0), w_audio=0.0, k=1)
    assert res[0][0] == "a"
